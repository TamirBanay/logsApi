...
void setup()
{
  ...
  serverAsyn.on("/update", HTTP_GET, [](AsyncWebServerRequest *request)
                {     updateFirmware();        
 });
...
void updateFirmware() {
  ...
  // Optionally, connect to WiFi if not already connected
  ...
  // Disable task watchdog for the current task
  esp_task_wdt_delete(xTaskGetCurrentTaskHandle());

  WiFiClient client;
  t_httpUpdate_return ret = httpUpdate.update(client, "https://alerm-api-9ededfd9b760.herokuapp.com/api/update");
  ...
  // Re-enable task watchdog for the current task if necessary
